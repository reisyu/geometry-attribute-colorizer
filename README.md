...
- **メモ** — オブジェクトを選ぶと属性インスペクタにメモ欄が出ます。点検の所見などを
  書き留めると、CSVのNote列として書き出されます。読み込み直せば戻ります。
  書いたメモはブラウザにも自動で控えられ、同じデータを読み込み直すと復元を提案します。
  「メモの有無」で色分けすれば、記入済みの箇所が図で一望できます（記入済みが赤、未記入はグレー）。
