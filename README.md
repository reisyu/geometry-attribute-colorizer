...
- **分類を付ける(石種・エリア)** — 計算では出せない分類を手で付けられます。
  選択肢は事前に決めず、**その列で使われている値がそのまま候補**になり、新しい値は
  入力した瞬間に候補に加わります。語彙はCSVに入るので、前回のCSVを読み込めば戻ります。
  表記ゆれは後から統合できます。未入力は灰色で表示され、塗り残しが一目で分かります
- **まとめて選ぶ(ブラシ)** — 選択モードでなぞると複数のオブジェクトを選べます。
  「孕み出している一角」のように数値では絞り込めないまとまりを選び、
  **まとめてメモを書く**ためのものです。選んでいないものは薄くなり、範囲が一目で分かります。
  1回のなぞり単位で取り消せます
...
見た目の設定(背景の不透明度、傾きラインの色と太さ、ラベルの文字サイズ、
SVG書き出しで含める要素、ライト/ダーク、投影方式など)は、
**次に開いたときも引き継がれます**。
